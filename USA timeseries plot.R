...
filename <- "us-counties-20200827.csv"
...
pl <- ggplot(data = country, aes(x=date, y=cases)) +
  ...
  scale_x_date(breaks = scales::pretty_breaks(n=8), limits = c(ymd("2020-02-01"), ymd("2020-09-03")), minor_breaks = NULL) +
  scale_y_continuous(limits=c(0,6e6), breaks = seq(from=0, to=6, by=1)*1e6, labels = function(x){paste0(x/1e6, "m")}, minor_breaks = NULL) +
  ggtitle(label = "USA Cumulative Cases") + theme(plot.title = element_text(hjust=0.5)) +
  geom_segment(aes(xend=ymd("2020-09-03"), yend=cases), linetype=2, color="blue") + 
  geom_text(aes(x=ymd("2020-09-03"), label=cases), hjust=0) + 
...
pla <- ggplot(data = country, aes(x=date, y=cases)) +
  ...
  scale_x_date(breaks = scales::pretty_breaks(n=8), limits = c(ymd("2020-02-01"), ymd("2020-09-03")), minor_breaks = NULL) +
  scale_y_continuous(limits=c(0,6e6), breaks = seq(from=0, to=6, by=1)*1e6, labels = function(x){paste0(x/1e6, "m")}, minor_breaks = NULL) +
  ggtitle(label = "USA Cumulative Cases, Linear") + theme(plot.title = element_text(hjust=0.5)) +
  transition_reveal(date) +
  ease_aes("linear") + 
  geom_segment(aes(xend=ymd("2020-09-02"), yend=cases), linetype=2, color="blue") + 
  geom_text(aes(x=ymd("2020-09-03"), label=as.character(round(cases, 0))), hjust=0) + 
  coord_cartesian(clip="off")
...
pla <- ggplot(data = country, aes(x=date, y=cases)) +
  ...
  ggtitle(label = "USA Cumulative Cases, Log10") + theme(plot.title = element_text(hjust=0.5)) +
  transition_reveal(date) +
  ease_aes("linear") + 
  geom_segment(aes(xend=ymd("2020-09-02"), yend=cases), linetype=2, color="blue") + 
  geom_text(aes(x=ymd("2020-09-03"), label=as.character(round(cases, 0))), hjust=0) + 
...
pla <- ggplot(data = country_deltas, aes(x=date, y=cases)) +
  ...
  ggtitle(label = "USA Daily Cases, Linear (weekly rolling average)") + theme(plot.title = element_text(hjust=0.5)) +
  transition_reveal(date) +
  ease_aes("linear") + 
  geom_segment(aes(xend=ymd("2020-09-02"), yend=cases), linetype=2, color="blue") + 
  geom_text(aes(x=ymd("2020-09-03"), label=as.character(round(cases, 0))), hjust=0) + 
...
pla <- ggplot(data = country_deltas, aes(x=date, y=cases)) +
  ...
  ggtitle(label = "USA Daily Cases, Log10 (weekly rolling average)") + theme(plot.title = element_text(hjust=0.5)) +
  transition_reveal(date) +
  ease_aes("linear") + 
  geom_segment(aes(xend=ymd("2020-09-02"), yend=cases), linetype=2, color="blue") + 
  geom_text(aes(x=ymd("2020-09-03"), label=as.character(round(cases, 0))), hjust=0) + 
...
anim_save(filename = here("sample-output", paste0("USA-20200827-deltalog10.mp4")),
          animation=pla, renderer=av_renderer(), duration = 30+3,
          fps=24, detail=1, width=1920*2/3, height=1080*2/3, end_pause = 3*24)
